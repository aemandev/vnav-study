...
class WaypointFollower {
  ...
  void generateOptimizedTrajectory(geometry_msgs::PoseArray const& poseArray) {
    ...
    mav_trajectory_generation::Vertex start_position(D), end_position(D), middle(D);
    mav_trajectory_generation::Vertex::Vector vertices;
    mav_trajectory_generation::Vertex start_yaw(1), end_yaw(1), middle_yaw(1);
    ...
    for (auto i = 0; i < poseArray.poses.size(); ++i) {
      ...
      //     **** FILL IN HERE ***
      // //
      temp_position << poseArray.poses[i].position.x,
          poseArray.poses[i].position.y, poseArray.poses[i].position.z;
      middle.addConstraint(POSITION, temp_position);
      vertices.push_back(middle);
      //
      // Populate yaw_vertices (for the waypoint yaw angles)
      //
      //
      temp_yaw = tf::getYaw(poseArray.poses[i].orientation);
      if (temp_yaw > 2*M_PI)
        temp_yaw -= 2*M_PI;
      else if (temp_yaw < -2*M_PI)
        temp_yaw += 2*M_PI;
      middle_yaw.addConstraint(ORIENTATION, temp_yaw);
    ...
    int list_size = poseArray.poses.size() -1;
    last_pos << poseArray.poses[list_size].position.x,
        poseArray.poses[list_size].position.y,
        poseArray.poses[list_size].position.z;
    end_position.makeStartOrEnd(last_pos, SNAP);
  ...
  void publishDesiredState(ros::TimerEvent const& ev) {
    ...
    double dt = ros::Time::now().toSec() - trajectoryStartTime.toSec();
    next_point.time_from_start = ros::Duration(dt);
    ...
    desired_pose.transform.translation.x = des_position[0];
    desired_pose.transform.translation.y = des_position[1];
    desired_pose.transform.translation.z = des_position[2];
    velocity.linear.x = des_velocity[0];
    velocity.linear.y = des_velocity[1];
    velocity.linear.z = des_velocity[2];
    tf2::Quaternion q;
    q.setRPY(0,0,des_orientation[0]);
    ...
    next_point.transforms[0] = desired_pose.transform;
    ...
    // velocity.linear.z = des_velocity[2];
    // tf2::Quaternion q;
    // q.setX(des_orientation[0]);
    // q.setY(des_orientation[1]);
    // q.setZ(des_orientation[2]);
    // q.setW(des_orientation[3]);
    // desired_pose.transform.rotation.x = q.x();
    // desired_pose.transform.rotation.y = q.y();
    // desired_pose.transform.rotation.z = q.z();
    // desired_pose.transform.rotation.w = q.w();
