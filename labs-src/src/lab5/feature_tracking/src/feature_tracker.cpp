...
void FeatureTracker::trackFeatures(const cv::Mat &img_1,
  ...
  // imwrite("keypoints_1.png", img_keypoints1);
  // imwrite("keypoints_2.png", img_keypoints2);
  ...
  // Extract the Keypoints corresponding to the good matches
  std::vector<KeyPoint> good_keypoints_1;
  std::vector<KeyPoint> good_keypoints_2;
  for (auto const& match : good_matches) {
    good_keypoints_1.push_back(keypoints_1[match.queryIdx]);
    good_keypoints_2.push_back(keypoints_2[match.trainIdx]);
    }
  matched_kp_1_kp_2->first = good_keypoints_1;
  matched_kp_1_kp_2->second = good_keypoints_2;

  inlierMaskComputation(good_keypoints_1, good_keypoints_2, &inlier_mask);
  drawInliers(img_1,
              img_2,
              keypoints_1, keypoints_2,
              good_matches,
  ...
  // ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~  ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
  //
  unsigned int num_inliers = std::accumulate(inlier_mask.begin(), inlier_mask.end(), 0);
  // 
  // For this part, you will need to:
...
void FeatureTracker::drawMatches(const cv::Mat &img_1,
  ...
  // cv::namedWindow("tracked_featuresssssss", cv::WINDOW_NORMAL);
  cv::Mat img_matches;
  //  Print the size of the keypoints and stuff
  ...
  // imshow("tracked_featuresssssss", img_matches);
...
void FeatureTracker::drawInliers(const cv::Mat &img_1,
                                 const cv::Mat &img_2,
                                 const std::vector<KeyPoint> &keypoints_1,
                                 const std::vector<KeyPoint> &keypoints_2,
                                 const std::vector<cv::DMatch> &matches,
