...
void evaluateRPE(const tf::Transform& gt_t_prev_frame,
  ...
  // ***************************** begin solution *****************************
  // Get the L2 norm of the distance between the estimated and ground truth displacement
  tf::Vector3 est_trans = est_relative_pose.getOrigin();
  tf::Vector3 gt_trans = gt_relative_pose.getOrigin();
  // Normalize the vector
  gt_trans = gt_trans/gt_trans.length();
  tf::Vector3 est_diff = est_trans - gt_trans;
  double translation_diff = est_diff.length();

  // Get the L2 norm of the difference between the orientation of the two matricies
  tf::Matrix3x3 est_rot = est_relative_pose.getBasis();
  tf::Matrix3x3 gt_rot = gt_relative_pose.getBasis();
  Eigen::Matrix3d est_rot_quat, gt_rot_quat;
  matrixTFToEigen(est_rot, est_rot_quat);   
  matrixTFToEigen(gt_rot, gt_rot_quat);
  // Normalize quats
  est_rot_quat = est_rot_quat/est_rot_quat.norm();
  gt_rot_quat = gt_rot_quat/gt_rot_quat.norm();
  Eigen::Matrix3d rot_diff = (est_rot_quat.inverse()*gt_rot_quat).normalized();
  Eigen::Vector3d axis;
  double angle;
  Eigen::AngleAxisd angle_axis(rot_diff);
  axis = Eigen::Vector3d(angle_axis.axis());
...
void cameraCallback(const sensor_msgs::ImageConstPtr &rgb_msg, const sensor_msgs::ImageConstPtr &depth_msg) {
  ...
  geometry_msgs::PoseStamped prev_pose_estimation;
  tf::poseTFToMsg(tf::Pose::getIdentity(), pose_estimation.pose);
  tf::poseTFToMsg(tf::Pose::getIdentity(), prev_pose_estimation.pose);
  ...
  case 2: {
    ...
    if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
      ...
      if (!FLAGS_use_ransac) {
        ...
        // create a TranslationOnlySacProblem
        std::shared_ptr<RansacProblemGivenRot>
            translationproblem_ptr(
              new opengv::sac_problems::relative_pose::TranslationOnlySacProblem(
                adapter_mono, false));
        // run ransac
        ransac.sac_model_ = translationproblem_ptr;
        ransac.threshold_ = 2.0*(1.0 - cos(atan(sqrt(2.0)*0.5/800.0)));
        ransac.max_iterations_ = 50;
        ransac.computeModel();
        // get the result
  ...
  case 3: {
    ...
    if (adapter_3d.getNumberCorrespondences() >= min_nr_of_correspondences) {
      if (!FLAGS_use_ransac){
        ...
        // (TODO) With RANSAC
        // ***************************** begin solution *****************************
        opengv::transformation_t threept_transformation = opengv::point_cloud::threept_arun(adapter_3d);
        opengv::transformation_t nonlinear_transformation = opengv::point_cloud::optimize_nonlinear(adapter_3d);
        opengv::sac::Ransac<RansacProblem3D> ransac;
        std::shared_ptr<RansacProblem3D>
            pointcloudproblem_ptr(
              new opengv::sac_problems::point_cloud::PointCloudSacProblem(
                adapter_3d));

        ransac.sac_model_ = pointcloudproblem_ptr;
        ransac.threshold_ =.1;
        ransac.max_iterations_ = 50;
        ransac.computeModel();
  ...
  prev_pose_estimation = pose_estimation; 
}
